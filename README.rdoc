...
Was created, because original Ancestry is somehow sleeping.

This fork is aimed to make it Rails 4 compatible.
Deprecated messages was removed.
Also there may be some Rails 2 (find_all and similar)
present, will be removed later (no deprecated messages for them).

Second fix is, that it is possible to use `path` or `ancestors`
in model callbacks. It is a little hack with .except(:where).
Because of this, there are currently some minor problems with sorting.

So it is now possible to use `create` method instead of `new.save`.
