module Ancestry
  module InstanceMethods
    ...
    def ancestors depth_options = {}
      self.base_class.except(:where).scope_depth(depth_options, depth).ordered_by_ancestry.where(ancestor_conditions)
    end
    ...
    def path depth_options = {}
      self.base_class.except(:where).scope_depth(depth_options, depth).ordered_by_ancestry.where(path_conditions)
    end
    ...
    def children
      self.base_class.except(:where).where(child_conditions)
    end
    ...
    def siblings
      self.base_class.except(:where).where(sibling_conditions)
    end
    ...
    def descendants depth_options = {}
      self.base_class.except(:where).ordered_by_ancestry.scope_depth(depth_options, depth).where(descendant_conditions)
    end
    ...
    def subtree depth_options = {}
      self.base_class.except(:where).ordered_by_ancestry.scope_depth(depth_options, depth).where(subtree_conditions)
    end
